# Project Rules
Your task is to develop the Obsidian plugin in this folder. Assume the role of an expert in this area. Make sure you follow all Obsidian plugin guidelines (unless they fundamentally conflict with the core idea of the plugin) so that when I publish a release, the https://community.obsidian.md/plugins/ page doesn't flag any errors or warnings (e.g., never add "!important" to `styles.css`, etc.):
```
Plugin guidelines
...
NOTE: After adding any functionality (even a small visual change), you must automatically add a very concise and simple description (length proportional to the change) to `CHANGELOG.md` under the latest header as a bullet point. This ensures that when I'm ready to publish, I won't have to remember what was added throughout the conversation and will have a ready list.

If the user asks you to prepare for publication by sending the single keyword "pub", then appropriately update the following files: `manifest.json`, `versions.json`, `CHANGELOG.md` (ensure the bullet points are fine, and there is a proper title with today's date), and `README.md` (based on `CHANGELOG.md`). Then, in the chat window, provide 3 proposals for the best possible English commit message (accurate, as short and concise as possible). You are forbidden from committing and pushing (the user will do this manually)!

Also, use English exclusively for all code comments and docs (you may use Polish or other languages ONLY in chat window). When adding comments, think ahead—don't add comments like "new feature here", but rather write them timelessly, specifically, and simply.

FINAL VERIFICATION: Ensure that all documentation is perfectly aligned with the current implementation. If you see any outdated comments, description, or examples in the code, update them immediately. The code itself is the "single source of truth". If a feature has changed, the documentation must be updated to match. This includes comments in the code, README, and any other documentation files.